...
// +kubebuilder:rbac:groups=application.newrelic.kubeform.com,resources=settings,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=application.newrelic.kubeform.com,resources=settings/status,verbs=get;update;patch

func (r *SettingsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("settings", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Settings")
...
func (r *SettingsReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &applicationv1alpha1.Settings{}); err != nil {
			klog.Error(err, "unable to set up auditor", applicationv1alpha1.Settings{}.APIVersion, applicationv1alpha1.Settings{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&applicationv1alpha1.Settings{}).
