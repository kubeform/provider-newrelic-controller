...
// ToMetricsRuleReconciler reconciles a ToMetricsRule object
type ToMetricsRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=events.newrelic.kubeform.com,resources=tometricsrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=events.newrelic.kubeform.com,resources=tometricsrules/status,verbs=get;update;patch

func (r *ToMetricsRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("tometricsrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ToMetricsRule")
...
func (r *ToMetricsRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &eventsv1alpha1.ToMetricsRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", eventsv1alpha1.ToMetricsRule{}.APIVersion, eventsv1alpha1.ToMetricsRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&eventsv1alpha1.ToMetricsRule{}).
