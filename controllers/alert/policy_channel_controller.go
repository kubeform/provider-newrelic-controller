...
// PolicyChannelReconciler reconciles a PolicyChannel object
type PolicyChannelReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=alert.newrelic.kubeform.com,resources=policychannels,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=alert.newrelic.kubeform.com,resources=policychannels/status,verbs=get;update;patch

func (r *PolicyChannelReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("policychannel", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PolicyChannel")
...
func (r *PolicyChannelReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &alertv1alpha1.PolicyChannel{}); err != nil {
			klog.Error(err, "unable to set up auditor", alertv1alpha1.PolicyChannel{}.APIVersion, alertv1alpha1.PolicyChannel{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&alertv1alpha1.PolicyChannel{}).
