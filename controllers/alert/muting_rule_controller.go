...
// MutingRuleReconciler reconciles a MutingRule object
type MutingRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=alert.newrelic.kubeform.com,resources=mutingrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=alert.newrelic.kubeform.com,resources=mutingrules/status,verbs=get;update;patch

func (r *MutingRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("mutingrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MutingRule")
...
func (r *MutingRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &alertv1alpha1.MutingRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", alertv1alpha1.MutingRule{}.APIVersion, alertv1alpha1.MutingRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&alertv1alpha1.MutingRule{}).
