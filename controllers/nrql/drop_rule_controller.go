...
// DropRuleReconciler reconciles a DropRule object
type DropRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=nrql.newrelic.kubeform.com,resources=droprules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=nrql.newrelic.kubeform.com,resources=droprules/status,verbs=get;update;patch

func (r *DropRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("droprule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DropRule")
...
func (r *DropRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &nrqlv1alpha1.DropRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", nrqlv1alpha1.DropRule{}.APIVersion, nrqlv1alpha1.DropRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&nrqlv1alpha1.DropRule{}).
