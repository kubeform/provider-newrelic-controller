...
// MultilocationAlertConditionReconciler reconciles a MultilocationAlertCondition object
type MultilocationAlertConditionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=synthetics.newrelic.kubeform.com,resources=multilocationalertconditions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=synthetics.newrelic.kubeform.com,resources=multilocationalertconditions/status,verbs=get;update;patch

func (r *MultilocationAlertConditionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("multilocationalertcondition", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MultilocationAlertCondition")
...
func (r *MultilocationAlertConditionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &syntheticsv1alpha1.MultilocationAlertCondition{}); err != nil {
			klog.Error(err, "unable to set up auditor", syntheticsv1alpha1.MultilocationAlertCondition{}.APIVersion, syntheticsv1alpha1.MultilocationAlertCondition{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&syntheticsv1alpha1.MultilocationAlertCondition{}).
