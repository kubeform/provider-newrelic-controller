...
// SecureCredentialReconciler reconciles a SecureCredential object
type SecureCredentialReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=synthetics.newrelic.kubeform.com,resources=securecredentials,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=synthetics.newrelic.kubeform.com,resources=securecredentials/status,verbs=get;update;patch

func (r *SecureCredentialReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("securecredential", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SecureCredential")
...
func (r *SecureCredentialReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &syntheticsv1alpha1.SecureCredential{}); err != nil {
			klog.Error(err, "unable to set up auditor", syntheticsv1alpha1.SecureCredential{}.APIVersion, syntheticsv1alpha1.SecureCredential{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&syntheticsv1alpha1.SecureCredential{}).
...
func (r *SecureCredentialReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for SecureCredential")
			return nil
		}
		...
		resourceList := &syntheticsv1alpha1.SecureCredentialList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list SecureCredential")
			return nil
		}
