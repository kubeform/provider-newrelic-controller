...
// MonitorScriptReconciler reconciles a MonitorScript object
type MonitorScriptReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=synthetics.newrelic.kubeform.com,resources=monitorscripts,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=synthetics.newrelic.kubeform.com,resources=monitorscripts/status,verbs=get;update;patch

func (r *MonitorScriptReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("monitorscript", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MonitorScript")
...
func (r *MonitorScriptReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &syntheticsv1alpha1.MonitorScript{}); err != nil {
			klog.Error(err, "unable to set up auditor", syntheticsv1alpha1.MonitorScript{}.APIVersion, syntheticsv1alpha1.MonitorScript{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&syntheticsv1alpha1.MonitorScript{}).
...
func (r *MonitorScriptReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for MonitorScript")
			return nil
		}
		...
		resourceList := &syntheticsv1alpha1.MonitorScriptList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list MonitorScript")
			return nil
		}
