...
// +kubebuilder:rbac:groups=synthetics.newrelic.kubeform.com,resources=alertconditions,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=synthetics.newrelic.kubeform.com,resources=alertconditions/status,verbs=get;update;patch

func (r *AlertConditionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("alertcondition", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AlertCondition")
...
func (r *AlertConditionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &syntheticsv1alpha1.AlertCondition{}); err != nil {
			klog.Error(err, "unable to set up auditor", syntheticsv1alpha1.AlertCondition{}.APIVersion, syntheticsv1alpha1.AlertCondition{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&syntheticsv1alpha1.AlertCondition{}).
