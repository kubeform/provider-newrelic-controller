...
// DashboardRawReconciler reconciles a DashboardRaw object
type DashboardRawReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=one.newrelic.kubeform.com,resources=dashboardraws,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=one.newrelic.kubeform.com,resources=dashboardraws/status,verbs=get;update;patch

func (r *DashboardRawReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("dashboardraw", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DashboardRaw")
...
func (r *DashboardRawReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &onev1alpha1.DashboardRaw{}); err != nil {
			klog.Error(err, "unable to set up auditor", onev1alpha1.DashboardRaw{}.APIVersion, onev1alpha1.DashboardRaw{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&onev1alpha1.DashboardRaw{}).
