...
// LevelReconciler reconciles a Level object
type LevelReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=service.newrelic.kubeform.com,resources=levels,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=service.newrelic.kubeform.com,resources=levels/status,verbs=get;update;patch

func (r *LevelReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("level", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Level")
...
func (r *LevelReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &servicev1alpha1.Level{}); err != nil {
			klog.Error(err, "unable to set up auditor", servicev1alpha1.Level{}.APIVersion, servicev1alpha1.Level{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&servicev1alpha1.Level{}).
