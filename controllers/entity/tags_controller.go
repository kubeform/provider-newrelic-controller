...
// TagsReconciler reconciles a Tags object
type TagsReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=entity.newrelic.kubeform.com,resources=tags,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=entity.newrelic.kubeform.com,resources=tags/status,verbs=get;update;patch

func (r *TagsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("tags", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Tags")
...
func (r *TagsReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &entityv1alpha1.Tags{}); err != nil {
			klog.Error(err, "unable to set up auditor", entityv1alpha1.Tags{}.APIVersion, entityv1alpha1.Tags{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&entityv1alpha1.Tags{}).
